...
class TypenaviresService {
  ...
  Future<List<Typenavires>> fetchFromApi() async {
    final response = await http.get(Uri.parse('https://ton-api.com/api/v1/typenavires'));
  ...
  Future<void> insert(Typenavires item) async {
    final db = await _db;
    await db.insert('typenavires', item.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }
